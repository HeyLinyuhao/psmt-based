...
class Tensorboard:
    def __init__(self, config, online=False, root_dir="./"):
        os.environ['WANDB_API_KEY'] = "8160f72f0d677e3fd3090eeeddd86dc8bbaff820"
    ...
    def update_wandb_recons_image(self, images,
                           recon,
                           img_number=4):

        images = self.de_normalize(images)
        wandb.log({"images_gt": [wandb.Image(j, caption="id {}".format(str(i)))
                              for i, j in enumerate(images[:img_number])], "global_step": self.current_step})

        wandb.log({"recovered": [wandb.Image(j, caption="id {}".format(str(i)))
                                    for i, j in enumerate(self.de_normalize(recon[:img_number, :, :]))],
                   "global_step": self.current_step})
