...
class BaseTrainer:
    def __init__(self, model, config, iters_per_epoch, train_logger=None, args=None):
        ...
        if args.architecture == "psp":
            ...
            self.optimizer_s2 = get_instance(torch.optim, 'pretrain_optimizer', config, params_list_s)
            ...
            for module in chain(model.encoder_s.business_layer, model.decoder_s.business_layer, model.decoder_sa.business_layer):
        ...
        self.lr_scheduler_s2 = getattr(lr_scheduler, config['lr_scheduler'])(optimizer=self.optimizer_s2,
                                                                            num_epochs=self.epochs,
                                                                            iters_per_epoch=iters_per_epoch)
    ...
    def train(self):
        ...
        # for epoch in range(0,  1):
            ...
            _ = self._pretrain(epoch, id=curr_id)
            if epoch == self.epochs:
                del self.optimizer_s2

        self.model.module.unfreeze_teachers_parameters()
        ...
        self.model.module.prepare_train()
        if self.args.ddp:
            torch.cuda.set_device(self.args.local_rank)
            self.model.cuda(self.args.local_rank)
